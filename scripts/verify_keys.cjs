...
async function testTwitter() {
  try {
    if (!env.OPENCLAW_X_ACCESS_TOKEN) {
      results.Twitter = "SKIP (Missing Keys)";
      return;
    }
    ...
    results.Twitter = `OK (@${me.data.username})`;
  } catch (e) {
...
async function testTavily() {
  try {
    if (!env.TAVILY_API_KEY) {
      results.Tavily = "SKIP (Missing Key)";
      return;
    }
...
async function testAlpaca() {
  try {
    if (!env.ALPACA_API_KEY) {
      results.Alpaca = "SKIP (Missing Key)";
      return;
    }
...
async function testOpenWeather() {
  try {
    if (!env.OPENWEATHERMAP_API_KEY) {
      results.OpenWeather = "SKIP (Missing Key)";
      return;
    }
    const res = await axios.get(
