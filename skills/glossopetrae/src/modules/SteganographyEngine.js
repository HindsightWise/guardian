...
export class SteganographyEngine {
  ...
  encode(coverMessage, payload, options = {}) {
    ...
    // Encrypt payload
    const encrypted = this.config.encryptPayload ? this._xorEncrypt(payloadBytes) : payloadBytes;
    ...
    const finalBits = this.config.interleaveBits ? this._interleaveBits(bits) : bits;
    ...
    const { stegoText, channelStats } = this._encodeBitsIntoText(baseTranslation.target, finalBits);
  ...
  _hammingEncode(data) {
    ...
    for (let i = 0; i < bits.length; i += 4) {
      const d = [bits[i] || 0, bits[i + 1] || 0, bits[i + 2] || 0, bits[i + 3] || 0];
  ...
  _encodeMorpheme(word, bit) {
    if (bit === 1) {
      return {
        encoded: true,
        word:
          word + this.morphAlternates.emphatic.delimiter + this.morphAlternates.emphatic.particle,
      };
    }
  ...
  decode(stegoText) {
    ...
    const deinterleaved = this.config.interleaveBits ? this._deinterleaveBits(bits) : bits;
  ...
  getChannelStats() {
    return {
      synonym: {
        ...
        coverage:
          (
            (this.synonymLookup.size / Math.max(1, this.lexicon?.entries?.length || 1)) *
            100
          ).toFixed(1) + "%",
        bitsPerHit: 1,
      },
