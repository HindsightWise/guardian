...
export class PhonemeSelector {
  ...
  _generateConsonants() {
    const inventory = [];
    const targetCount = this.random.int(
      this.config.consonantCount[0],
      this.config.consonantCount[1],
    );
    ...
    if (
      this.config.allowAffricates &&
      inventory.some((c) => CONSONANTS.stops.some((s) => s.ipa === c.ipa))
    ) {
  ...
  _getUniversalConsonants() {
    ...
    universals.push(
      CONSONANTS.stops.find((s) => s.place === "bilabial" && s.voice === "voiceless"),
    );

    // At least one alveolar stop
    universals.push(
      CONSONANTS.stops.find((s) => s.place === "alveolar" && s.voice === "voiceless"),
    );
  ...
  _selectStops() {
    ...
    if (this.config.allowVoicedStops) {
      ...
      for (const stop of voicedStops) {
        ...
        const hasVoiceless = selected.some(
          (s) => s.place === stop.place && s.voice === "voiceless",
        );
  ...
  _selectNasals(currentInventory) {
    ...
    for (const nasal of nasals) {
      ...
      const hasStop =
        stopPlaces.has(nasal.place) || nasal.place === "bilabial" || nasal.place === "alveolar";
