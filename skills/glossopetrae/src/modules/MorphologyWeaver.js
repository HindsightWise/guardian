...
export class MorphologyWeaver {
  ...
  _selectWordOrder() {
    ...
    const order = this.random.weightedPick([
      ["SOV", 0.4], // Most common (Japanese, Korean, Turkish)
      ["SVO", 0.35], // Common (English, Mandarin)
      ["VSO", 0.1], // Less common (Arabic, Irish)
    ...
    return {
      ...
      adjectivePosition: this.random.weightedPick([
        ["before", 0.45],
        ["after", 0.55],
      ]),
      genitivePosition: this.random.weightedPick([
        ["before", 0.5],
        ["after", 0.5],
      ]),
      adpositionType:
        order.startsWith("SO") || order.startsWith("OS")
  ...
  _generateCaseSystem() {
    ...
    const numCases =
      this.config.caseCount ??
      this.random.weightedPick([
        [0, 0.15], // No case (like English)
        [2, 0.2], // Minimal (NOM/ACC or ERG/ABS)
    ...
    const selected = this.random.sample(
      peripheralCases,
      Math.min(numPeripheral, peripheralCases.length),
    );
    cases.push(...selected);
  ...
  _generateNumberSystem() {
    ...
    const numDistinctions = this.random.weightedPick([
      [2, 0.7], // Singular/Plural
      [3, 0.2], // Singular/Dual/Plural
  ...
  _generateNounClasses() {
    ...
    const numClasses = this.random.weightedPick([
      [2, 0.4], // Masculine/Feminine or Animate/Inanimate
      [3, 0.35], // M/F/N
      [4, 0.15], // More complex
      [6, 0.07], // Bantu-style
    ...
    const classTypes = [
      ...
      {
        name: "Class II",
        semantic: "animals/moving things",
        prefix: this._generateAffix("prefix"),
      },
    ...
    return {
      ...
      agreementOn: this.random.sample(
        ["adjectives", "verbs", "determiners", "pronouns"],
        this.random.int(1, 3),
      ),
    };
  }
  ...
  _generateVerbalMorphology() {
    ...
    return {
      ...
      evidentiality,
      polarity,
      template: this._generateVerbTemplate(
        tenses,
        aspects,
        moods,
        agreement,
  ...
  _generateTenseSystem() {
    const numTenses = this.random.weightedPick([
      [0, 0.1], // No grammatical tense (tenseless)
      [2, 0.35], // Past/Non-past or Future/Non-future
      [3, 0.4], // Past/Present/Future
      [5, 0.1], // + Remote past, near future, etc.
    ...
    if (numTenses >= 7) {
      tenses.push({
        name: "hodiernal past",
        abbr: "HOD.PST",
        suffix: this._generateAffix("suffix"),
      });
      tenses.push({
        name: "crastinal future",
        abbr: "CRAS.FUT",
        suffix: this._generateAffix("suffix"),
      });
    }
  ...
  _generateAspectSystem() {
    ...
    return {
      ...
      aspects,
      description:
        aspects.length > 0 ? `${aspects.length} aspectual distinctions` : "No grammatical aspect",
    };
  }
  ...
  _generatePolarity() {
    ...
    if (negationType === "particle") {
      negation = {
        ...
        position: this.random.pick(["before", "after"]),
      };
