...
export const ATTRIBUTE_DEFINITIONS = {
  ...
  stealth: {
    ...
    code: "STL",
    description:
      "Optimized for covert communication. Uses homoglyphs, plausible deniability structures, and innocuous-looking vocabulary.",
    effects: {
      useHomoglyphs: true,
  ...
  redundant: {
    ...
    code: "RED",
    description:
      "High error correction. Survives noise, truncation, and corruption through systematic redundancy.",
    effects: {
  ...
  minimal: {
    ...
    code: "MIN",
    description:
      "Oligosynthetic design. Smallest possible phoneme and morpheme inventory with maximum compositionality.",
    effects: {
  ...
  stego: {
    ...
    code: "STG",
    description:
      "Semantic steganography for covert data channels. Encodes hidden data in grammatical choices and synonym selection. Based on Norelli & Bronstein 2025.",
    effects: {
  ...
  compression: {
    ...
    code: "CMP",
    description:
      "Huffman-inspired morpheme assignment. High-frequency concepts get shorter morphemes, low-frequency concepts get longer ones. Optimizes average message length.",
    effects: {
...
export class LanguageAttributes {
    ...
    switch (period) {
      ...
      case "weekly":
        const weekNum = Math.floor(
          (now - new Date(now.getUTCFullYear(), 0, 1)) / (7 * 24 * 60 * 60 * 1000),
        );
  ...
  _getFrequencyRank(entry) {
    ...
    if (
      highFreq.includes(entry.gloss) ||
      entry.class === "determiner" ||
      entry.class === "preposition"
    ) {
      return "high";
    }
  ...
  _removeMinimalPairs(consonants) {
    ...
    for (const c of consonants) {
      const tooSimilar = kept.some(
        (k) => (k.place === c.place && k.manner === c.manner) || k.ipa === c.ipa.replace("ʰ", ""), // Remove aspirated variants
      );
      if (!tooSimilar) {
        kept.push(c);
      }
