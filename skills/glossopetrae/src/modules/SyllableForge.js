...
export class SyllableForge {
  ...
  _generateClusterRules() {
    ...
    return {
      ...
      description:
        "Consonants must rise in sonority toward the vowel (onset) and fall away from it (coda)",
  ...
  _generatePhonologicalProcesses() {
    ...
    if (this.config.vowelHarmony) {
      const harmonyType =
        this.config.harmonyType || this.random.pick(["backness", "rounding", "height"]);
      processes.push({
        name: "Vowel Harmony",
        type: harmonyType,
    ...
    if (this.random.bool(0.35)) {
      ...
      const epentheticCandidates = this.phonology.vowels.filter(
        (v) => v.height === "mid" || v.ipa === "i",
      );
      const epentheticVowel =
        epentheticCandidates.length > 0
