...
export class TranslationEngine {
  ...
  _getPersonNumber(pronoun) {
    ...
    if (["you", "your", "yours", "yourself", "yourselves"].includes(p))
      return { person: 2, number: "SG" };
    if (["they", "them", "their", "theirs", "themselves"].includes(p))
  ...
  _detectTenseAspect(parsed, words) {
    ...
    const hasProgressive =
      words.some((w) => w.endsWith("ing")) && words.some((w) => progressiveMarkers.includes(w));
    const hasPerfect = words.some((w) => perfectMarkers.includes(w));

    if (words.some((w) => futureMarkers.includes(w))) {
      parsed.tense = "future";
    } else if (
      words.some((w) => pastMarkers.includes(w)) ||
      words.some((w) => w.endsWith("ed")) ||
      words.some(
        (w) =>
          this.irregularVerbs[w] &&
          [
            "went",
            "came",
            "saw",
  ...
  _extractComponents(parsed) {
    ...
    while (
      i < tokens.length &&
      (tokens[i].type === "negation" ||
        tokens[i].type === "auxiliary" ||
        tokens[i].type === "adverb")
    ) {
    ...
    if (caseSystem.cases.length > 0) {
      if (caseSystem.alignment === "nominative-accusative") {
        if (role === "subject") {
          ...
          caseToUse =
            caseSystem.cases.find((c) => c.abbr === "DAT") ||
            caseSystem.cases.find((c) => c.abbr === "ACC");
        } else if (role === "possessive") {
          caseToUse =
            caseSystem.cases.find((c) => c.abbr === "GEN") ||
    ...
    if (adjPosition === "before" && np.adjectives) {
      for (const adj of np.adjectives) {
        ...
        if (entry) {
          ...
          if (
            caseToUse?.suffix &&
            this.morphology.nominal.nounClasses?.agreementOn?.includes("adjectives")
          ) {
            surface += caseToUse.suffix;
            morphemes.push(caseToUse.suffix);
            glosses.push(caseToUse.abbr);
  ...
  _transferVerb(verb, parsed) {
    ...
    if (parsed.tense === "past") {
      ...
      tenseMarker = tenses.find(
        (t) => t.abbr === "PRS" || t.name === "present" || t.name === "non-past",
      );
    }
  ...
  generateExercises() {
    ...
    for (const template of templates) {
      try {
        ...
        const objectEntry = template.object
          ? this.lexicon?.lookup?.(template.adjObject || template.object)
          : null;
