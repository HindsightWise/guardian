...
export class LexiconGenerator {
  ...
  generate() {
    ...
    return {
      ...
      getByClass: (wordClass) =>
        Array.from(lexiconMap.values()).filter((e) => e.class === wordClass),
    };
  }
  ...
  _generateParadigmExamples() {
    ...
    const nouns = Array.from(this.lexicon.values()).filter(
      (e) => e.class === "noun" && e.paradigm?.forms,
    );
    if (nouns.length > 0) {
    ...
    const verbs = Array.from(this.lexicon.values()).filter(
      (e) => e.class === "verb" && e.paradigm?.forms,
    );
    if (verbs.length > 0) {
  ...
  _generateStats() {
    ...
    return {
      ...
      byClass: {
        ...
        other: entries.filter((e) => !["noun", "verb", "adjective", "pronoun"].includes(e.class))
          .length,
      },
