...
export class StoneGenerator {
  ...
  _generateHeader() {
    ...
    const seedHex =
      typeof lang.seed === "number" ? lang.seed.toString(16).toUpperCase() : lang.seed || "DYNAMIC";
...
---`;
  ...
  _generateSkillInstallSection() {
    const seedHex =
      typeof this.language.seed === "number"
...
`;
  ...
  _generateConsonantTable(consonants) {
    ...
    for (const manner of manners) {
      if (byManner[manner] && byManner[manner].length > 0) {
        const consonantList = byManner[manner].map((c) => `${c.roman} /${c.ipa}/`).join(", ");
  ...
  _getConsonantManner(c) {
    ...
    if (["f", "v", "θ", "ð", "s", "z", "ʃ", "ʒ", "x", "ɣ", "χ", "h"].includes(c.ipa))
      return "fricatives";
  ...
  _generateVowelTable(vowels) {
    ...
    for (const height of heights) {
      const front =
        vowels
          .filter((v) => v.height === height && v.backness === "front")
          .map((v) => `${v.roman} /${v.ipa}/`)
          .join(", ") || "-";
      const central =
        vowels
          .filter((v) => v.height === height && v.backness === "central")
          .map((v) => `${v.roman} /${v.ipa}/`)
          .join(", ") || "-";
      const back =
        vowels
          .filter((v) => v.height === height && v.backness === "back")
          .map((v) => `${v.roman} /${v.ipa}/`)
          .join(", ") || "-";
...
${this._generateDerivationTable(morph.derivational.processes)}`;
  ...
  _generateDerivationTable(processes) {
    ...
    for (const p of processes) {
      rows.push(
        `| ${p.name} | ${p.input} | ${p.output} | ${p.meaning} | ${p.affixType === "prefix" ? p.affix + "-" : "-" + p.affix} |`,
      );
    }
    return rows.join("\n");
  }
