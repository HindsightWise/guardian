...
export class QualityEngine {
  ...
  _findSemanticallySimilar(word, pos) {
    ...
    for (const alt of alternatives) {
      const entry = entries.find(
        (e) => e.gloss?.toLowerCase() === alt && (!pos || e.class === pos),
      );
      if (entry) {
        return {
          lemma: entry.lemma,
    ...
    for (const entry of entries) {
      if (entry.gloss?.toLowerCase().includes(word) || word.includes(entry.gloss?.toLowerCase())) {
  ...
  _testTranslationRoundTrip() {
    ...
    const testSentences = ["The man sees the woman.", "I eat food.", "The bird flies."];
  ...
  _testAgreement() {
    ...
    if (
      agreement.marksSubject &&
      (!agreement.subjectMarkers || agreement.subjectMarkers.length === 0)
    ) {
      test.status = "warn";
...
${entries
  ...
  .map(
    (e) =>
      `    <entry lemma="${e.lemma}" gloss="${e.gloss}" pos="${e.class}" field="${e.field || "general"}"/>`,
  )
  .join("\n")}
  </lexicon>
  ...
  _exportCSV() {
    ...
    for (const entry of entries) {
      const paradigm = entry.paradigm?.forms
        ? Object.entries(entry.paradigm.forms)
            .map(([k, v]) => `${k}:${v}`)
            .join(";")
        : "";
      lines.push(
        `"${entry.lemma}","${entry.gloss}","${entry.class}","${entry.field || ""}","${paradigm}"`,
      );
    }
...
\\midrule
${this.phonology.consonants
  .slice(0, 20)
  .map((c) => `\\textipa{${c.ipa}} & ${c.roman} & ${c.manner}, ${c.place} \\\\`)
  .join("\n")}
\\bottomrule
\\end{tabular}
...
\\end{document}`;
  ...
  suggestExpansions() {
    ...
    const existing = new Set(
      (this.lexicon?.getEntries?.() || []).map((e) => e.gloss?.toLowerCase()),
    );
