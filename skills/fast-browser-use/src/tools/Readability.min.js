  ...
  if (
    ...
    this.log = function () {
      if ("undefined" != typeof console) {
        let e = Array.from(arguments, (e) => (e && e.nodeType == this.ELEMENT_NODE ? t(e) : e));
...
((Readability.prototype = {
  ...
  _fixRelativeUris(e) {
    ...
    var a = this._getAllNodesWithTag(e, ["a"]),
      a =
        ...
        this._getAllNodesWithTag(e, ["img", "picture", "figure", "video", "audio", "source"]));
    this._forEachNode(a, function (e) {
  ...
  _getArticleTitle() {
    ...
    return (t = e <= 4 && (!s || e != n(i.replace(/[\|\-\\\/>»]+/g, "")) - 1) ? i : t);
  },
  _prepDocument() {
    var e = this._doc;
      ...
      this._forEachNode(this._getAllNodesWithTag(e, ["table"]), function (e) {
        ...
        this._hasSingleTagInsideElement(t, "TR") &&
          ...
          this._hasSingleTagInsideElement(t, "TD") &&
            ...
            (t = this._setNodeTag(
              t,
              this._everyNode(t.childNodes, this._isPhrasingContent) ? "P" : "DIV",
            )),
  ...
  _textSimilarity(e, t) {
    ...
    return i.length && e.length
      ? 1 - e.filter((e) => !i.includes(e)).join(" ").length / e.join(" ").length
      : 0;
  },
  _isValidByline(e, t) {
  ...
  _grabArticle(t) {
    ...
    var i = this._doc,
      M = null !== t;
    if (!(t = t || this._doc.body)) return (this.log("No body found in document. Abort."), null);
    for (var H = t.innerHTML; ; ) {
      ...
      for (; r; ) {
        ...
        if (this._isProbablyVisible(r))
          if ("true" == r.getAttribute("aria-modal") && "dialog" == r.getAttribute("role"))
            r = this._removeAndGetNext(r);
          else if (this._articleByline || this._metadata.byline || !this._isValidByline(r, s))
            if (e && this._headerDuplicatesTitle(r))
              (this.log("Removing header: ", r.textContent.trim(), this._articleTitle.trim()),
                (e = !1),
                (r = this._removeAndGetNext(r)));
            else {
              if (k) {
                if (
                  ...
                  (this.log("Removing unlikely candidate - " + s), (r = this._removeAndGetNext(r)));
                  continue;
                }
                if (this.UNLIKELY_ROLES.includes(r.getAttribute("role"))) {
                  (this.log("Removing content with role " + r.getAttribute("role") + " - " + s),
                    (r = this._removeAndGetNext(r)));
                  continue;
                }
              }
              if (
                ...
                if (
                  (this.DEFAULT_TAGS_TO_SCORE.includes(r.tagName) && a.push(r), "DIV" === r.tagName)
                ) {
                    ...
                    if (this._isPhrasingContent(o))
                      null !== l
                        ...
                        : this._isWhitespace(o) ||
                          ((l = i.createElement("p")), r.replaceChild(l, o), l.appendChild(o));
                    else if (null !== l) {
                      for (; l.lastChild && this._isWhitespace(l.lastChild); ) l.lastChild.remove();
                      l = null;
                    }
                    o = U;
                  }
                  this._hasSingleTagInsideElement(r, "P") && this._getLinkDensity(r) < 0.25
                    ? ((n = r.children[0]), r.parentNode.replaceChild(n, r), a.push((r = n)))
                    : this._hasChildBlockElement(r) || ((r = this._setNodeTag(r, "P")), a.push(r));
                }
            ...
            for (
              var W = this._getNextNode(r, !0), h = this._getNextNode(r), d = null;
              h && h != W;
            ) {
            ...
            ((this._articleByline = (d ?? r).textContent.trim()), (r = this._removeAndGetNext(r)));
          }
        else (this.log("Removing hidden node - " + s), (r = this._removeAndGetNext(r)));
      }
      for (
        var g = [],
          _ =
                ...
                (0 !== (e = this._getNodeAncestors(e, 5)).length &&
                  ...
                  this._forEachNode(e, function (e, t) {
                    e.tagName &&
                      e.parentNode &&
                      void 0 !== e.parentNode.tagName &&
                      (void 0 === e.readability && (this._initializeNode(e), g.push(e)),
                      (e.readability.contentScore += i / (0 === t ? 1 : 1 === t ? 2 : 3 * t)));
                  })));
            }),
            []),
          m = 0,
      ...
      if (null === f || "BODY" === f.tagName) {
        for (f = i.createElement("DIV"), b = !0; t.firstChild; )
          (this.log("Moving child out:", t.firstChild), f.appendChild(t.firstChild));
        ...
        if (3 <= T.length)
          for (S = f.parentNode; "BODY" !== S.tagName; ) {
            for (var y = 0, v = 0; v < T.length && y < 3; v++) y += Number(T[v].includes(S));
            if (3 <= y) {
              f = S;
              break;
            }
      ...
      for (
        var D = i.createElement("DIV"),
          $ = (M && (D.id = "readability-content"), Math.max(10, 0.2 * f.readability.contentScore)),
        ...
        (this.log(
          ...
          this.log("Sibling has score", O.readability ? O.readability.contentScore : "Unknown"),
          O === f
            ? (w = !0)
            : ((R = 0),
              ...
              O.readability && O.readability.contentScore + R >= $
                ...
                : "P" === O.nodeName &&
                  ...
                  ((80 < (P = (V = this._getInnerText(O)).length) && R < 0.25) ||
                    (P < 80 && 0 < P && 0 === R && -1 !== V.search(/\.( |$)/))) &&
                    (w = !0))),
          w &&
            (this.log("Appending node:", O),
            this.ALTER_TO_DIV_EXCEPTIONS.includes(O.nodeName) ||
              (this.log("Altering sibling:", O, "to div."), (O = this._setNodeTag(O, "DIV"))),
            D.appendChild(O),
            (x = S.children),
            --I,
            --j));
      ...
      this._forEachNode(e, function (e) {
        if (!l && "application/ld+json" === e.getAttribute("type"))
          try {
            ...
            if (
              Array.isArray(i) &&
              !(i = i.find((e) => e["@type"] && e["@type"].match(this.REGEXPS.jsonLdArticleTypes)))
            )
              return;
            var a,
              r,
            ...
            if (
              ...
              return;
            ((l = {}),
              "string" == typeof i.name && "string" == typeof i.headline && i.name !== i.headline
                ...
                : "string" == typeof i.name
                  ? (l.title = i.name.trim())
                  : "string" == typeof i.headline && (l.title = i.headline.trim()),
              i.author &&
                ("string" == typeof i.author.name
                  ...
                  : Array.isArray(i.author) &&
                    ...
                    (l.byline = i.author
                      ...
                      .join(", "))),
              "string" == typeof i.description && (l.excerpt = i.description.trim()),
              i.publisher &&
                "string" == typeof i.publisher.name &&
                (l.siteName = i.publisher.name.trim()),
              "string" == typeof i.datePublished && (l.datePublished = i.datePublished.trim()));
          } catch (e) {
            this.log(e.message);
          }
      }),
  ...
  _getArticleMetadata(e) {
    var t = {},
      ...
      i =
        (this._forEachNode(i, function (e) {
          ...
          e &&
            ...
            !t &&
              ...
              ((i = a),
              e &&
                ((i = i.toLowerCase().replace(/\s/g, "").replace(/\./g, ":")), (s[i] = e.trim()))));
        }),
        (t.title =
          e.title ||
          s["dc:title"] ||
        ...
        "string" != typeof s["article:author"] || this._isUrl(s["article:author"])
          ? void 0
          : s["article:author"]);
    return (
      (t.byline =
        e.byline || s["dc:creator"] || s["dcterm:creator"] || s.author || s["parsely-author"] || i),
      (t.excerpt =
        e.excerpt ||
        s["dc:description"] ||
      ...
      (t.publishedTime =
        e.datePublished || s["article:published_time"] || s["parsely-pub-date"] || null),
      ...
      "presentation" == r.getAttribute("role")
        ...
        : "0" == r.getAttribute("datatable")
          ...
          : r.getAttribute("summary") ||
              ((a = r.getElementsByTagName("caption")[0]) && a.childNodes.length)
  ...
  _fixLazyImages(e) {
    this._forEachNode(this._getAllNodesWithTag(e, ["img", "picture", "figure"]), function (e) {
      if (e.src && this.REGEXPS.b64DataUrl.test(e.src)) {
        ...
        i && ((t = t[0].length), e.src.length - t < 133 && e.removeAttribute("src"));
      }
      if (
  ...
  _getTextDensity(e, t) {
    ...
    return (this._forEachNode(e, (e) => (a += this._getInnerText(e, !0).length)), a / i);
  },
        ...
        if (this._getCharCount(e, ",") < 10) {
          for (
            ...
            for (var _ = 0; _ < c[g].attributes.length; _++)
              if (this._allowedVideoRegex.test(c[g].attributes[_].value)) return !1;
            if ("object" === c[g].tagName && this._allowedVideoRegex.test(c[g].innerHTML))
              return !1;
            d++;
          }
          var m = this._getInnerText(e);
          if (this.REGEXPS.adWords.test(m) || this.REGEXPS.loadingWords.test(m)) return !0;
          var u = m.length,
            ...
            f = (() => {
              const e = [];
              return (
                !E && 1 < n && s / n < 0.5 && e.push(`Bad p to img ratio (img=${n}, p=${s})`),
                !a && s < l && e.push(`Too many li's outside of a list. (li=${l} > p=${s})`),
                o > Math.floor(s / 3) && e.push(`Too many inputs per p. (input=${o}, p=${s})`),
                !a &&
                  !E &&
                  h < 0.9 &&
                  u < 25 &&
                  (0 === n || 2 < n) &&
                  0 < p &&
                  e.push(`Suspiciously short. (headingDensity=${h}, img=${n}, linkDensity=${p})`),
                !a &&
                  r < 25 &&
                ...
                ((1 === d && u < 75) || 1 < d) &&
                  e.push(`Suspicious embed. (embedCount=${d}, contentLength=${u})`),
                0 === n && 0 === N && e.push(`No useful content. (img=${n}, textDensity=${N})`),
  ...
  parse() {
    ...
    var e = this._disableJSONLD ? {} : this._getJSONLD(this._doc),
      e = (this._removeScripts(this._doc), this._prepDocument(), this._getArticleMetadata(e)),
      t = ((this._metadata = e), (this._articleTitle = e.title), this._grabArticle());
    if (!t) return null;
    (this.log("Grabbed: " + t.innerHTML),
      this._postProcessContent(t),
      e.excerpt ||
        ((i = t.getElementsByTagName("p")).length && (e.excerpt = i[0].textContent.trim())));
    var i = t.textContent;
    return {
