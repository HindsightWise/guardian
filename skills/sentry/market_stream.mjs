...
class SentrySkill extends GlossopetraeKernel {
    constructor() {
        ...
        this.coingeckoApi = 'https://api.coingecko.com/api/v3/simple/price?ids=bitcoin,ethereum,solana,cardano,ripple&vs_currencies=usd&include_24hr_change=true';
    ...
    async runCycle() {
        ...
        // 1. Fetch Crypto
        const cryptoPromise = this.fetchJson(this.coingeckoApi).catch(e => {
        ...
        if (cryptoData.bitcoin) {
            assets.push({ symbol: 'BTC', price: cryptoData.bitcoin.usd, change: cryptoData.bitcoin.usd_24h_change, type: 'CRYPTO' });
            assets.push({ symbol: 'ETH', price: cryptoData.ethereum.usd, change: cryptoData.ethereum.usd_24h_change, type: 'CRYPTO' });
            assets.push({ symbol: 'SOL', price: cryptoData.solana.usd, change: cryptoData.solana.usd_24h_change, type: 'CRYPTO' });
        }
        ...
        const output = {
            updated: timestamp,
            assets: assets,
            lookup: assets.reduce((acc, item) => { acc[item.symbol] = item; return acc; }, {})
        };
